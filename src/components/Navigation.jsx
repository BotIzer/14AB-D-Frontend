  ...
  return (
    ...
    >
      ...
      {searchResults.users.length > 0 && searchResults.users.map((item) => (
        <Dropdown.Item onPointerDown={()=>HandleSearchNavigation(`/user/${item.name}`)} className='d-flex justify-content-center'  key={item.name}>{item.name.length > 25 ? `${item.name.substring(0,20)}... (user)` : `${item.name} (user)`}</Dropdown.Item>
      ))}
      {searchResults.forums.length > 0 && searchResults.forums.map((item) => (
        <Dropdown.Item onPointerDown={()=>HandleSearchNavigation(`/forums/${item.name}/${item.id}`)} className='d-flex justify-content-center'  key={item.name}>{item.name.length > 25 ? `${item.name.substring(0,20)}... (forum)` : `${item.name} (forum)`}</Dropdown.Item>
      ))}
